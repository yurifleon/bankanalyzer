...
4. **`read_transactions(input_csv, date_col, text_col, debit_col, credit_col, card_col=None)`** (line 322) — ties the above together into a list of transaction dicts: `{date, month, description, vendor, card_number, debit, credit, net, row_number, row}`. `debit`/`credit`/`net` stay `Decimal` here and everywhere downstream until output.


4b. **`categorize_transaction(vendor, description, net)`** (line 295) — labels each transaction with one of 13 spending categories, or `Uncategorized`. It runs once per row inside `read_transactions()`, immediately after `clean_vendor_name()`. Keywords in `_CATEGORY_RULES` (line 190) are matched as **whole words** against a haystack built from the cleaned vendor name *and* the raw description; searching the raw description is essential because `clean_vendor_name()` drops `TRANSFER`, `PAYMENT`, `DEPOSIT` and `ACH` as `NOISE_WORDS` — exactly the words that identify the structural categories. **The table's order is load-bearing**: structural rules (Transfers, Income, Fees & Interest) sit above merchant rules so a payroll deposit isn't claimed by a merchant keyword; `Subscriptions` sits above `Utilities` because `_VENDOR_REPLACEMENTS` rewrites Apple's `BILINTERNET` to `INTERNET`; `Shopping` is last because its keywords are broadest. Unmatched rows fall back to `Income` if the transaction is a net credit, otherwise `Uncategorized` — except that `_REFUND_KEYWORDS` (returned merchandise, refunds) short-circuits the `Income` fallback to `Uncategorized`, so money coming back from a purchase is never counted as earnings. `NON_SPEND_CATEGORIES` (line 267) names the two categories held out of spend-oriented views.

5. **Summarization** — `summarize_by_month_vendor()` (line 506) groups by `(month, vendor, card_number)`; `summarize_month_totals()` (line 573) groups by `month` alone; `top_10_per_month()` (line 702) takes the month-vendor summary and ranks it. `write_workbook()` (line 875) calls `summarize_by_month_vendor()` exactly once and feeds the same result to both the *Monthly Grouped* sheet and `top_10_per_month()`, avoiding a second full pass over the transaction list.

6. **`detect_recurring_activity(transactions, min_occurrences=3)`** (line 627) — a separate analysis, not part of the main summarization chain. It groups by `(vendor, card_number)`, requires ≥3 transactions, then computes the coefficient of variation (stdev / mean) of both the day-gaps between transactions and the transaction amounts. A group counts as recurring only if the gap's CV is within `_RECURRING_INTERVAL_TOLERANCE` (0.35); the matched gap is classified against `_FREQUENCY_BANDS` (Weekly/Biweekly/Monthly/Quarterly/Annual by average day-gap range). Amount CV within `_FIXED_AMOUNT_TOLERANCE` (0.15) is labeled "Fixed Amount", otherwise "Variable Amount". Direction (`Income / Credit` vs `Expense / Charge`) comes from the sign of the summed `net`.

7. **`write_workbook(transactions, output_xlsx)`** (line 875) — produces the `.xlsx` with five sheets (*Monthly Totals*, *Monthly Grouped*, *Top 10 Per Month*, *Monthly by Category*, *Recurring Activity*), frozen header row, auto-filter. This is the **only place** (besides the web app's chart helpers, below) where `Decimal` gets cast to `float` — openpyxl needs native numeric types.

8. **`main()`** (line 917) — the CLI entry point: resolves column indices from the chosen `--profile` or explicit overrides, runs the pipeline, optionally writes a second filtered workbook and/or a subset CSV, and can print recurring activity / top-10 vendors to the console.
...
2. `POST /analyze` — saves the upload under a fresh `uuid4()` subdirectory of `UPLOAD_DIR` (so concurrent uploads never collide), resolves profile/column overrides from form fields, calls `analyzer.read_transactions()`, then `analyzer.write_workbook()` for the full dataset and again for a search/month-filtered subset if requested. It computes several view-model structures — `compute_summary`, `compute_top_patterns`, the two chart-data helpers below, plus `analyzer.summarize_month_totals`, `summarize_by_month_vendor`, `summarize_by_month_category`, `top_10_per_month`, `get_available_months`, `detect_recurring_activity` — and renders `templates/results.html`.
...
```python
{
    ...
    "category": str,              # categorize_transaction(vendor, description, net)
...
`tests/test_bank_csv_monthly_dual_profile_cardnum.py` (unittest) covers only the core analyzer module — `parse_amount`, `parse_date`, `clean_vendor_name`, `safe_filename`, `detect_recurring_activity`, and friends. `tests/test_web_app.py` covers `web_app.build_category_data` only; the rest of `web_app.py` (routes, `compute_summary`, `parse_optional_int`, the download guard) still has **no automated coverage** and is verified manually by running the dev server and exercising routes. This means:
...
- **New category rule:** add a keyword to the right tuple in `_CATEGORY_RULES` (line 190). The table is evaluated top-to-bottom, first match wins, so **position matters** — see the ordering comment above it. Keywords match as whole words against the cleaned vendor *and* the raw description.
